    ...
    <div className="bg-black flex flex-col items-center space-y-6 my-8 w-full px-4">
      {step === 1 && (
        <form onSubmit={(e) => { e.preventDefault(); if (date && selectedTime) { setStep(2); sendMetaEvent('InitiateCheckout'); } }} className="flex flex-col items-center space-y-6">
          ...
          <button type="submit" className="bg-yellow text-black font-semibold px-8 py-3 rounded hover:bg-yellow-600 w-full sm:w-auto">Próximo</button>
        </form>
      )}

      {step === 2 && (
        <form onSubmit={handleSubmit} className="bg-[#333] p-10 rounded-lg shadow-lg flex flex-col items-center space-y-6">
          <h2 className="text-white font-bold text-2xl md:text-3xl text-center mb-8">Preencha seus Dados</h2>
          <input type="text" placeholder="Nome" value={name} onChange={(e) => setName(e.target.value)} className="w-full max-w-lg px-4 py-2 rounded border" required />
          <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full max-w-lg px-4 py-2 rounded border" required />
          <input type="text" placeholder="WhatsApp" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} className="w-full max-w-lg px-4 py-2 rounded border" required />
          <button type="submit" className={`${isLoading ? 'bg-gray-500' : 'bg-yellow-500'} text-black font-semibold px-8 py-3 rounded hover:bg-yellow-600 w-full`} disabled={isLoading}>
